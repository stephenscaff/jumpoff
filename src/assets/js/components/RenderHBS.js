...
const RenderHBS = (() => {
  ...
  return {
     ...
     */
    render(data, renderEl, hbsTemplate, replaceContents, callback) {
      RenderHBS.getTemplate(hbsTemplate, function(template) {
        if (replaceContents && renderEl.hasChildNodes()) {
          while (renderEl.hasChildNodes()) {
            renderEl.removeChild(renderEl.firstChild)
          }
        }
